...
export type TemplateName = 'password-reset' | 'welcome-client' | '2fa-enabled' | 'test'
...
function wrap(brandColor: string, content: string): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${styles(brandColor)}</style></head><body><div class="container">${header()}<div class="content">${content}</div>${footer()}</div></body></html>`
}
...
export async function renderTemplate<T extends TemplateName>(name: T, vars: TemplateVars[T]): Promise<Rendered> {
  ...
  switch (name) {
    ...
    case 'welcome-client': {
      ...
      const html = wrap(brandColor, `<h1>Welcome, ${escapeHtml(v.name)}</h1><p>Your clinician has set up your rehab portal account. Click the button below to set your password and get started:</p><p style="text-align:center;margin:24px 0;"><a class="btn" href="${escapeHtml(v.setupUrl)}">Set up my account</a></p><p class="muted">This link expires in 7 days. If it doesn't work, ask your clinician to send a new one.</p><p class="muted">Once you've set your password, save it in your password manager for easy login.</p>`)
    ...
    case '2fa-enabled': {
      ...
      const html = wrap(brandColor, `<h1>2FA is now active</h1><p>Hi ${escapeHtml(v.name)},</p><p>Two-factor authentication was enabled on your account on ${escapeHtml(v.when)}.</p><p>If this wasn't you, please contact your clinician immediately and change your password.</p>`)
